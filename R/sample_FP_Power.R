...
#'
#' @param rejections A binary vector; \code{rejection[i] = 1} means the null hypothesis is
...
#' @param test.statistic A vector of test statistics; e.g., t-statistics or p-values that
#' are used to determine whether or not to reject the null hypothesis.
#' @param reject.threshold A list whose first element is the rejection criteria, e.g., the
...
#' @param FP,TP Binary vectors of false positive and true positive indicators, respectively.
...
#' @param B A vector of "true" parameter values. For inference purposes, this can be a vector
...
#' @note The default operating approach is that the null hypothesis is \code{B[i] = 0} for
...
#' @export
sample_FP_Power = function(rejections = NULL, FP = NULL, TP = NULL,
                           test.statistic = NULL, reject.threshold = NULL,
                           B = NULL, B.incl.B0 = TRUE, full.summary = FALSE) {
