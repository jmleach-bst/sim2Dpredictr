...
#' @param type Specifies either sparse (\code{type = "sparse"}) or full (\code{type = "full"}) proximity matrix.
...
proximity_builder <- function(im.res, neighborhood = "ar1",
  ...
  # determine weights
  if (weight == "distance") {
    nb.xy.old <- dplyr::mutate(nb.xy.old,
  ...
  if (type == "sparse") {
    ...
    for (i in 1:nrow(nb.xy.old)) {
      adj.mat[nb.xy.old$location.index[i], nb.xy.old$nb.index[i]] <- nb.xy.old$weights[i]
    }
    return(adj.mat)
  }
